...
def get_icd_from_nih(name): #根据疾病term获得icdcode
...
def process_all():
	...
	t1 = time()
	disease_hit, disease_all = 0,0 ### disease hit icd && drug hit smiles, hit记录非空数\all记录总数
	...
	for file in tqdm(input_file_lst[:]):
		...
		result = xml_file_2_tuple(file) #conditions, label, why_stop, None
		## 0.1 & 0.2 
		if len(result)==1:
	...
	for disease, cnt in disease2cnt:
		for word in disease.split():
			word_cnt[word] += cnt # 记录所有未找到的word和出现次数
