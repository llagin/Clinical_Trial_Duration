...
def xml_file_2_tuple(xml_file):
	...
	# 'detail','number_of_groups', 'enrollment', 'number_of_location'
	# try:
	# 	detail = '['+root.find('detailed_description').find('textblock').text+']'
	# except:
	# 	detail = '[]'
	...
	try:
		number_of_location = len(root.findall('location'))
	...
	try:
		# intervention_model
		result= root.find('study_design_info').find('intervention_model').text
		if result == 'Single Group Assignment':
			intervention_model = '10000'
		elif result == 'Crossover Assignment':
			intervention_model = '01000'
		elif result == 'Factorial Assignment':
			intervention_model = '00100'
		elif result == 'Sequential Assignment':
			intervention_model = '00010'
		elif result == 'Parallel Assignment':
			intervention_model = '00001'
		else:
	...
	try:
		# masking
		result = root.find('study_design_info').find('masking').text
		result = result.lower()
		care_provider = '0'
		investigator = '0'
		outcomes_assessor = '0'
		...
		if 'care_provider' in result:
			care_provider = '1'
		if 'investigator' in result:
			investigator = '1'
		if 'outcomes_assessor' in result:
			outcomes_assessor = '1'
	...
	try:
		...
		for outcome in primary_outcomes:
			outcomes.append(outcome.find('time_frame').text)
	...
	return nctid, status.lower(), why_stop.lower(), label, phase.lower(), conditions, drugs, title, criteria, summary, \
		enrollment, number_of_location, intervention_model, masking, primary_purpose, time_frames
...
def process_all():
	...
	with open(output_file, 'w', newline='', encoding='utf-8-sig') as csvfile:
		...
		for file in tqdm(input_file_lst[:]):
			...
			nctid, status, why_stop, label, phase, conditions, drugs, title, criteria, summary, \
				enrollment, number_of_location, intervention_model, masking, primary_purpose, time_frames = result
			...
			data_count += 1
#			nctid, status, why_stop, label, phase, conditions, drugs, title, criteria, summary, detail, number_of_groups, \
#				enrollment, number_of_location, intervention_model, masking, primary_purpose, time_frame

			# if nctid == 'NCT00058747':
			# 	print('result:', title)
			# 	title+='.'
				# exit()
			...
			# title,summary,detail,primary_purpose biobert;
			# number_of_groups,enrollment,number_of_location  number
			# intervention_model,masking onehot
			# time_frame text
	t2 = time()
