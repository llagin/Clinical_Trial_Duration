...
if __name__ == "__main__":
    ...
    date_df = pd.read_csv('data/ntcid_time.csv', sep='\t')

    raw_df = pd.read_csv('data/raw_data.csv', sep=',', dtype={'masking': str, 'intervention_model': str})
    # print(raw_df['masking'][:10])
    input_df = pd.merge(date_df, raw_df[['nctid', 'phase', 'diseases', 'drugs', 'criteria', 'title', 'summary',
                                         'enrollment', 'number_of_location', 'intervention_model', 'masking',
                                         'primary_purpose', 'time_frame', 'smiless', 'icdcodes']],
                        on='nctid', how='inner')
